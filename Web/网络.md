...
5. **cookie**
   ...
   3. 初次用浏览器访问某个网站的时候，当访问完毕后，对方服务器响应完毕后给你返回的响应头中会增加了一个叫做`set-cookie`的属性，这个`set-cookie`的值就是对方服务器对于你的唯一标识，会在这个唯一标识中存储了你这次的数据信息。当你再一次，也就是第二次访问这个网站，浏览器给对方发送的请求请求头中添加一个`set-cookie`属性，并加上你的`cookie`的`id`值，会告诉对方服务器你的`cookie`的`id`值，相应成功后，对方服务器一看请求头中的`cookie`值，我认识你，找出对应的`cookie`的`id`值存储的信息返回给你的浏览器。
